...
if ($_POST['rid']< SEPARADOR_ID_RESERVES) {
...
if ($mob!=$_POST['mobil']){
  $rid=$_POST['rid'];
  $mobil=$_POST['mobil'];
  $result= '<div class="alert alert-danger" style="color:#bb8888;font-size:26px;" ><i class="fa fa-exclamation-triangle" style="color:#bb8888;font-size:26px;"></i> Reserva no trobada</div>';
}
else{
  
  $b64=base64_encode($_POST['rid']."&".$_POST['mobil']);
  switch ($_POST['accio']) {
    case 'pay':
      header('Location: /cb-reserves/editar/pagament256.php?id='.$_POST['rid']);
exit();
      break;

    case 'cancel':
      header('Location: /reservar/realitzar-reserva/?rid='.$b64);
exit();
      break;
    case 'edit':
      header('Location: /reservar/realitzar-reserva/?rid='.$b64);
