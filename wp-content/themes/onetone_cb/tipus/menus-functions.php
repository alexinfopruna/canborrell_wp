...
add_action('init', 'cb_menus_register');  

function cb_menus_register() {
    $args = array(
        'label' => __('Menús i Carta', 'cb_backend'),
        'public' => true,
        'show_ui' => true,
...
add_action("admin_init", "cb_menus_title_settings");   

function cb_menus_title_settings(){
 //  echo "sssssssWWW";die();
    add_meta_box("mens_title_settings", "Menus", "cb_menus_title_config", "menus", "normal", "high");
}   

function cb_menus_title_config(){
        global $post;
        
       if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ) return $post_id;
        $custom = get_post_custom($post->ID);
		
		if(isset($custom["menus_content"][0])) $testimonial_content = $custom["menus_content"][0];
...
add_action('save_post', 'cb_save_menus_meta'); 

function cb_save_menus_meta(){
    global $post;  	
    ...
    if ( defined('DOING_AUTOSAVE') && DOING_AUTOSAVE ){
		...
		$post_metas = array('name','role','menus_content');
