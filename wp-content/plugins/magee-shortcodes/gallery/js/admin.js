jQuery(document).ready(function($){
	...
	if (jQuery('#magee_removeall').hasClass('premp6')) {var button = '<button class="media-modal-icon"></button>';}

	else {var button = '<button></button>';}
	
	jQuery('#magee_select').on('click', function(event){
		...
		file_frame.on('open', function() {
			...
			var ids = jQuery('#magee_perm_metadata').val();
			if (ids) {
				idsArray = ids.split(',');
		...
		file_frame.on('update', function() {
			...
			if (metadataString) {
				jQuery("#magee_perm_metadata").val(metadataString);
				jQuery("#featuredgallerydiv ul").html(imageHTML);
				jQuery('#magee_select').text('Edit Selection');
				jQuery('#magee_removeall').addClass('visible');
				setTimeout(function(){
	...
	jQuery('#featuredgallerydiv ul').on('click', 'button', function(event){
		...
		if (confirm('Are you sure you want to remove this image?')) {
			...
			var oldGallery = jQuery("#magee_perm_metadata").val();

			var newGallery = oldGallery.replace(','+removedImage,'').replace(removedImage+',','').replace(removedImage,'');

			jQuery(this).parent('li').remove();

			jQuery("#magee_perm_metadata").val(newGallery);

			if (newGallery == "") {

				jQuery('#magee_select').text('Select Images');

				jQuery('#magee_removeall').removeClass('visible');
	...
	jQuery('#magee_removeall').on('click', function(event){
		...
		if (confirm('Are you sure you want to remove all images?')) {
			...
			jQuery("#magee_perm_metadata").val("");
...
function ajaxUpdateTempMetaData() {
	...
	jQuery.ajax({
		...
		url : myAjax.ajaxurl,
		data : {
			action: "magee_update_temp", 
			magee_post_id: jQuery("#magee_perm_metadata").data("post_id"), 
			magee_temp_noncedata: jQuery("#magee_temp_noncedata").val(), 
			magee_temp_metadata: jQuery("#magee_perm_metadata").val()
		},
