...
if (typeof permisos === "undefined") permisos = 0;
if (typeof COMENSALS_AVIS_VARIACIO === "undefined") COMENSALS_AVIS_VARIACIO = 8;

if (browser_malo) {
  alert(
    "El sistema de reservas requiere Internet Explorer 7 o superior\n\nVersión utilizada:\n Microsoft Internet Explorer " +
      $.browser.version,
  );
...
$(function () {
  ...
  $(".navbar-toggle").click(function () {
    ...
    if ($("#navbar").is(":visible")) $("#navbar").hide("slow");
    else $("#navbar").show("slow");
  });
  ...
  $("#a_editar_reserva").click(function () {
    if ($("#form_contactar").is(":visible")) $("#form_contactar").toggle("low");
    $("#flogin").toggle("low");
  });

  $("#form_contactar.r-petita").hide();
  $("#a_consulta_online.r-petita").click(function () {
    if ($("#flogin").is(":visible")) $("#flogin").toggle("low");
  ...
  $("#cancel_reserva").click(function () {
    if (!confirm(l("Segur que vols eliminar la teva reserva?"))) return false;
    else return true;
  });
...
$("#terrassa").change(function () {
  ...
  //  $(".fr-seccio-quants").amaga();
  //  $(".fr-seccio-carta").show();
...
function totalPersones() {
  ...
  var cotxets = $("input[name='selectorCotxets']:checked").val()
    ? $("input[name='selectorCotxets']:checked").val()
    : "0";
  ...
  if (total >= PERSONES_GRUP) {
    ...
    if (confirm(l("REDIR_GRUPS"))) {
      window.location.href =
        "/reservar/reserva-grup/?a=redir&b=" +
        na +
        "&c=" +
        nj +
        "&d=" +
  ...
  $.post(
    ...
    function (dades) {
      ...
      if (!$("input[value='" + hora + "']"))
        alert(
          l(
            "Ho lamentem, no queda cap taula pel dia, hora i coberts que sol·licites",
          ),
        );
...
function updateClient() {
  ...
  $.post(
    GESTOR + "?a=recuperaClient&b=" + num + "&c=" + email,
    function (dades) {
      if ((dades == "false" || dades == "err0") && client_auto) {
        ...
        if (obj.id_reserva || obj.err) {
          ...
          $(".fr-seccio-client input[name='client_mobil']").removeAttr(
            "readonly",
          );
        ...
        $(".fr-seccio-client input[name='client_nom']").attr(
          "readonly",
          "readonly",
        );
        ...
        $(".fr-seccio-client input[name='client_telefon']").attr(
          "readonly",
          "readonly",
        );
        client_auto = true;
...
function updateResum() {
  ...
  $("#bt_reserva_consulta_online").click(function () {
    $.post(
      GESTOR + "?a=enviaIncidencia&b=" + $("input[name='id_reserva']").val(),
      { c: $("textarea[name='reserva_consulta_online']").val() },
      function (dades) {
...
function monta_calendari(selector) {
  ...
  if (!MARGE_DIES_RESERVA_ONLINE) {
    ...
    if (MAX_HORA_RESERVA_ONLINE.includes(":"))
      maxTime.setMinutes(parseInt(MAX_HORA_RESERVA_ONLINE.substring(3, 5)));
  ...
  $(selector).datepicker({
    beforeShowDay: function (date, inst) {
      ...
      if (
        ((date.getDay() == 1 || date.getDay() == 2 || llistanegra(date)) &&
          !llistablanca(date)) ||
        !taulaDisponible(date)
      ) {
        r[0] = false;
        r[1] = "maldia";
        r[2] = l("Tancat");
  ...
  $(selector).datepicker("option", $.datepicker.regional[lng]);

  if (!RDATA) {
    $(".ui-datepicker-calendar .ui-state-active").removeClass(
      "ui-state-active",
    );
...
function validacio() {
  $("#form-reserves").validate({
    ...
    rules: {
      ...
      client_email: {
        required: true,
        email: true,
        remote: "../../cb-reserves/taules/verify_email.php",
      },
      client_nom: "required",
    ...
    messages: {
      totalComensals: {
        ...
        max: l(
          "Si sou més de " +
            (PERSONES_GRUP - 1) +
            " comensals, selecciona la reserva per GRUPS",
        ),
      },
      ...
      client_email: {
        required: l("Dona´ns un email"),
        email: l("El format no és correcte"),
      },
  ...
  $("#form_contactar").validate({
    ...
    messages: {
      reserva_consulta_online: {
        required: l("Aquest camp és necessari"),
        minlength: l("Mínim 10 lletres"),
      },
      client_email: {
        required: l("Dona´ns un email"),
        email: l("El format no és correcte"),
...
function controlSubmit() {
  var loading =
    '<div style="height:320px"><img src="/cb-reserves/reservar/css/loading.gif"/></div>';
  if (browser_malo)
    ...
    $("#form-reserves").ajaxSubmit(function (dades) {
      ...
      if (obj.resposta == "ok") // RESPOSTA OK
      {
        ...
        if (obj.TPV == "TPV") {
          ...
          $("#popup").html(
            info +
              '<iframe id="frame-tpv" name="frame-tpv" style="width:100%;height:100px"></iframe>',
          );
          ...
          timer = setTimeout(function () {
            ...
            $.post(GESTOR + "?a=estatReserva&b=" + obj.idr, function (r) {
              ...
              if (d != 100) {
                ...
                $("#progress-pas-7 .glyphicon").removeClass(
                  "glyphicon-transfer",
                );
...
function resultat_estat(idr) {
  var desti =
    "/cb-reserves/taules/gestor_reserves.php?a=estat_reserva_online&b=" + idr;
...
function loginon() {
  ...
  $("#editar_reserva").click(function () {
    $("#editar_reserva")
      .stop()
      .animate({ "margin-top": "0px", height: "14px" }, 400, "swing", loginoff);
  });
}
function loginoff() {
...
function observacions_cotxets() {
  ...
  if (
    obs.match(
      /cotxet|carret|person|nen|comensal|cobert|cochecito|carrito|niñ|cubierto/gi,
    )
  ) {
...
function calc() {
  ...
  vent = window.open(
    "",
    "frame-tpv",
    "width=725,height=600,scrollbars=no,resizable=yes,status=yes,menubar=no,location=no",
  );
