...
if (!defined('FACTOR_IVA'))
  define('FACTOR_IVA', $factor_iva);
...
class Carta extends Gestor {
  ...
  public function __construct($db_connection_file = DB_CONNECTION_FILE, $usuari_minim = 16) {
    ...
    while ($row = mysqli_fetch_assoc($this->qry_result)) {
     ...
     // if ($id == "2017")        echo $id . " --- " . $row['carta_plats_preu'] . " --- " . $preuIVA . " ..........";
      $this->arPreusIVA[$id] = $preuIVA;
      $this->arLabelID[$id] = "{preu_" . $id . "}";


      $nom = $row['carta_plats_nom_' . $this->lng];
      if (empty($row['carta_plats_nom_' . $this->lng])) {
        if (l("titol_menu_" . $id, false) != ("titol_menu_" . $id))
          $nom = l("titol_menu_" . $id, false);
        else
          $nom = $row['carta_plats_nom_' . $this->lng_default];
      }

      $q++;
      $descripcio = l("menu_" . $id, false);

      if ($descripcio == "menu_" . $id)
        $descripcio = "";
...
// ET TORNA PREU AMB IVA
  public function preuPlat($id, $iva = true) {
    if (!$id)
      return FALSE;
    if ($iva)
      return($this->plats[$id]->preuIVA);
    else
      return($this->plats[$id]->preu);
  }
